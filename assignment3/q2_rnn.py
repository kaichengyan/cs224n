...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        if old_len < max_length:
            padded_sentence = sentence + [zero_vector] * (max_length - old_len)
            padded_labels = labels + [zero_label] * (max_length - old_len)
            masking = [True] * old_len + [False] * (max_length - old_len)
        else:
            padded_sentence = sentence[:max_length]
            padded_labels = labels[:max_length]
            masking = [True] * max_length
        ret.append((padded_sentence, padded_labels, masking))
        ### END YOUR CODE ###
    return ret
...
class RNNModel(NERModel):
    ...
    def add_embedding(self):
        ...
        embeddings = tf.nn.embedding_lookup(params, self.input_placeholder)
    ...
    def add_prediction_op(self):
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ...
                o, state = cell(x[:, time_step, :], state)
                o_drop = tf.nn.dropout(o, dropout_rate)
                pred = tf.nn.xw_plus_b(o_drop, U, b2)
                preds.append(pred)
                ### END YOUR CODE
